...
def process_file(input_path: str, output_path: str, args: argparse.Namespace):
...
def main():
    ...
    ap.add_argument("input", help="Input video file or directory")
    ap.add_argument("-o", "--output", default=None, help="Output file or directory (default: adds _desilenced suffix)")
    ap.add_argument("--recursive", action="store_true", help="Search for videos recursively if input is a directory")
    ap.add_argument("--extensions", default="mp4,mkv,mov,avi", help="Comma-separated list of video extensions to process in batch mode")
    ...
    if os.path.isdir(input_path):
        ...
        if not video_files:
            print(f"No video files found in {input_path} with extensions {extensions}")
            sys.exit(0)

        print(f"Batch mode: Found {len(video_files)} videos to process.")
        ...
        for i, vid in enumerate(video_files):
            ...
            out_file = os.path.join(out_dir, f"{name}_desilenced.mp4")
            
            # Skip if it's already a desilenced version we created previously
            if vid.endswith("_desilenced.mp4"):
                continue
                
            process_file(vid, out_file, args)
